...
export const Codeblock = ({contents}) => {
    ...
    const [selectedOption, setSelectedOption] = React.useState(Object.keys(contents)[0])
                ...
                <Options>
                    <StyledSelect value={selectedOption} onChange={e => setSelectedOption(e.target.value)}>
                        {Object.keys(contents).map((item) => {
                            return(
                                <option value={item} key={item}>{contents[item].name}</option>
                            )
                        })}
                    </StyledSelect>
                </Options>
                <CopyButton hover={hoverState} onMouseEnter={() => {setHoverState(true)}} onMouseLeave={() => {setHoverState(false)}} onClick={() => {navigator.clipboard.writeText(contents[selectedOption].content)}}>
